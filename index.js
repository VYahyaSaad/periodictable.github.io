...
const inpt1 = document.getElementById("inpts-1");
...
const elebtn = (n) => {
  main.innerHTML = "";
//   nf.innerHTML = "";
  let f = data.find((e) => e.number == n);
  second.innerHTML = ` 
    <button class="back shadow"  onclick="back()">
    <svg class="back-img" xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-arrow-left-short" viewBox="0 0 16 16">
    <path fill-rule="evenodd" d="M12 8a.5.5 0 0 1-.5.5H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5H11.5a.5.5 0 0 1 .5.5z"/>
    </svg> 
    <h3 class="back-head">Go Back</h3>  
    </button>
      <div class="prid-second" style="background-color: #${f.cpk_hex}">
        <h6>${f.number}</h6>
        <div class="prod-in">
          <h4 class="prid-head-s">${f.symbol}</h4>
          <h6 class="prid-tail-s">${f.name}</h6>
        </div>
      </div>
      <div>
    <model-viewer src="${f.bohr_model_3d}" alt="A 3D model of a car" shadow-intensity="1" camera-controls auto-rotate ar >
    </model-viewer>
      </div>
      <div class="prid-in-main">
        <div class="prid-in-con">
          <h4 class="txt-head">Name</h4>
          <h3 class="txt-tail">${f.name}</h3>
        </div>
        <div class="prid-in-con">
          <h4 class="txt-head">Symbol</h4>
          <h3 class="txt-tail">${f.symbol}</h3>
        </div>
        <div class="prid-in-con">
          <h4 class="txt-head">atomic no</h4>
          <h3 class="txt-tail">${f.number}</h3>
        </div>
        <div class="prid-in-con">
          <h4 class="txt-head">mass no</h4>
          <h3 class="txt-tail">${f.atomic_mass}</h3>
        </div>

        <div class="prid-in-con">
          <h4 class="txt-head">Type</h4>
          <h3 class="txt-tail">${f.category}</h3>
        </div>

        <div class="prid-in-con">
          <h4 class="txt-head">density</h4>
          <h3 class="txt-tail">${f.density}</h3>
        </div>

        <div class="prid-in-con">
          <h4 class="txt-head">melt</h4>
          <h3 class="txt-tail">${f.melt}</h3>
        </div>

        <div class="prid-in-con">
          <h4 class="txt-head">molar heat</h4>
          <h3 class="txt-tail">${f.molar_heat}</h3>
        </div>
        <div class="prid-in-con">
          <h4 class="txt-head">phase</h4>
          <h3 class="txt-tail">${f.phase}</h3>
        </div>

        <div class="prid-in-con">
          <h4 class="txt-head">period</h4>
          <h3 class="txt-tail">${f.period}</h3>
        </div>
        <div class="prid-in-con">
          <h4 class="txt-head">affinity</h4>
          <h3 class="txt-tail">${f.electron_affinity}</h3>
        </div>

        <div class="prid-in-con">
          <h4 class="txt-head">electronegativity</h4>
          <h3 class="txt-tail">${f.electronegativity_pauling}</h3>
        </div>
      </div>
      <div class="prid-in-con">
        <h4 class="txt-head">electronic configuration</h4>
        <h3 class="txt-tail">${f.electron_configuration}</h3>
      </div>
      <div class="prid-in-con">
        <h4 class="txt-head">discovered by</h4>
        <h3 class="txt-tail">${f.discovered_by}</h3>
      </div>
      ${
        f.named_by != null
          ? `<div class="prid-in-con">
        <h4 class="txt-head">named by</h4>
        <h3 class="txt-tail">${f.named_by}</h3>
      </div>`
          : ""
      }
      `;
...
function all() {
  ...
  data.map((e, i) => {
    n = 1;
    main.innerHTML += `
    <div class="prid-main" onclick="elebtn(${e.number})">
        <div class="prid-box" style="background-color:#${e.cpk_hex} ;">
        <h3>${e.number}</h3>
        <div class="prod-in">
            <h1 class="prid-head">${e.symbol}</h1>
            <h3 class="prid-tail">${e.name}</h3>
        </div>
      </div>
      </div>
    `;
...
select.addEventListener("change", () => {
  if (select.value === "number" || select.value === "mass") {
    document.getElementById("inpts-1").type = "number";
  } else {
    document.getElementById("inpts-1").type = "text";
  }
});
const btn = () => {
  let find;
  ...
  if (select.value == "number") {
    find = data.find((e) => e.number == inpt1.value);
    Number(inpt1.value) !== 0
      ? find !== undefined
        ? elebtn(find.number)
        : (nf.innerHTML =     `<div class="ntf-div">
        <img src="./notfound.svg"  class="ntf" alt="">
        <h3>No elements found</h3>
    </div>`)
      : all();
  } else if (select.value == "symbol") {
    find = data.find(
      (e) => e.symbol.toLowerCase() == inpt1.value.toLowerCase()
    );
    inpt1.value.toLowerCase() !== ""
      ? find !== undefined
        ? elebtn(find.number)
        : (nf.innerHTML =     `<div class="ntf-div">
        <img src="./notfound.svg"  class="ntf" alt="">
    ...
    find = data.find((e) => e.name.toLowerCase() == inpt1.value.toLowerCase());
    inpt1.value.toLowerCase() !== ""
      ? find !== undefined
        ? elebtn(find.number)
        : (nf.innerHTML = `<div class="ntf-div">
      ...
      : all();
  } else if (select.value == "elect") {
    find = data.find((e) => e.electron_configuration.toLowerCase() == inpt1.value.toLowerCase());
    inpt1.value.toLowerCase() !== ""
      ? find !== undefined
        ? elebtn(find.number)
        : (nf.innerHTML = `<div class="ntf-div">
    ...
    console.log(Number(inpt1.value).toFixed(0));
    find = data.find(
      (e) => e.atomic_mass.toFixed(0) == Number(inpt1.value).toFixed(0)
    );
    Number(inpt1.value).toFixed(0) !== 0
      ? find !== undefined
        ? elebtn(find.number)
        : (nf.innerHTML = `<div class="ntf-div">
        <img src="./notfound.svg"  class="ntf" alt="">
    ...
    if(select.value ==="categ"){
        document.getElementById(
          "err"
        ).innerHTML = `<h3 class="err-head shadow">Please Click and select a category</h3>
    `;
